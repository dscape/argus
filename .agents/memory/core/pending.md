...
- Autoresearch unresolved: `v884` is prepared in `autoresearch/train.py` and awaiting controller execution; next loop should compare it against the `props8 + minsep8` anchor (`v805/v806/v808/v811/v812/v816/v817/v818`) and the harmful lower-threshold regime (`v820/v839..v881`), then choose exactly one new adjacent cheap follow-up from the true bracket if needed; avoid `props15`, avoid further margin ULP search, and re-encode from restored `v545` baseline as needed.
