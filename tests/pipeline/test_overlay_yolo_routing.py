...
def test_fast_overlay_check_uses_default_yolo(monkeypatch) -> None:
    ...
    assert scanner.fast_overlay_check(frame) == expected


def test_detect_overlay_fast_pads_default_yolo_bbox(monkeypatch) -> None:
    frame = np.zeros((64, 64, 3), dtype=np.uint8)
    ...
    assert scanner.detect_overlay_fast(frame) == OverlayDetection(
        found=True,
        bbox=(2, 2, 36, 36),
        seed_bbox=(10, 10, 20, 20),
        score=0.9,
