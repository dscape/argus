...
async def get_transient_annotation(clip_path: str):
    return {
        "annotation": await run_in_threadpool(
            physical_train_service.get_transient_annotation,
            clip_path,
        )
    }
...
async def delete_transient_annotation(clip_path: str):
    deleted = await run_in_threadpool(
        physical_train_service.delete_transient_annotation,
        clip_path,
    )
    if not deleted:
...
async def track_corners(body: TrackCornersRequest):
    try:
        result = await run_in_threadpool(
            physical_train_service.track_corners,
            body.session_id,
...
async def save_transient_annotation(body: SaveTransientAnnotationRequest):
    return await run_in_threadpool(
        physical_train_service.save_transient_annotation,
        body.clip_path,
