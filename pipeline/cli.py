...
def cmd_split_clips(args):
    """Prepare a video-disjoint train/val dataset from generated clips."""
...
def cmd_inspect(args):
    ...
    for video_id, channel_handle, title in videos:
        ...
        try:
                ...
                with conn.cursor() as cur:
                    cur.execute(
                        ...
                        (
                            ...
                            else ("otb_only" if result.has_otb else None),
                            video_id,
                        ),
                    )
                    conn.commit()
            ...
            overlay_str = (
                f"overlay={result.overlay_score:.2f}" if result.has_overlay else "no overlay"
            )
...
def cmd_ai_extract_status(args):
    ...
    if os.path.isdir(cache_dir):
        ...
        for f in os.listdir(cache_dir):
            if not f.endswith(".pt"):
                continue
            try:
                data = torch.load(os.path.join(cache_dir, f), map_location="cpu", weights_only=True)
    ...
    print(
        f"  {cached}/{total} ({pct}%) — overlay={labels[0]}, otb={labels[1]}, "
...
def cmd_stats(args):
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            try:
                cur.execute("SELECT COUNT(*) FROM youtube_videos WHERE screening_status IS NULL")
...
def cmd_fetch_frames(args):
    ...
    for video_id, channel in videos:
        if args.resolution == "fullres":
            ...
            results = fetch_overlay_frames(video_id, hires=(args.resolution == "hires"))
        if results:
...
def main():
    ...
    p = subparsers.add_parser(
        "inspect",
        help="Inspect videos for overlay + OTB via frame extraction",
    )
    ...
    p = subparsers.add_parser(
        "split-clips",
        help="Prepare a video-disjoint train/val dataset from generated clips",
    )
    p.add_argument(
        "--clips-dir",
        type=str,
    ...
    p.add_argument(
        "--out-dir",
        type=str,
        default="data/argus/training_dataset",
    ...
    p.add_argument(
        "--random", action="store_true", help="Randomize video selection (avoids sampling bias)"
    )
