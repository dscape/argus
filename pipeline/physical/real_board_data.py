...
class PhysicalRealBoardDataset(Dataset[tuple[torch.Tensor, torch.Tensor]]):
    """Rectified real-board frames with replay-derived square labels."""
    ...
    def __getitem__(self, index: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        image_rgb = _frame_tensor_to_rgb(frame)
        rectified = rectify_board_image(
            image_rgb,
            list(row.corners),
            output_size=DEFAULT_BOARD_SIZE,
        )
        image_bgr = cv2.cvtColor(rectified, cv2.COLOR_RGB2BGR)
