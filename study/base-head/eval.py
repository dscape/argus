...
def main() -> None:
    ...
    piece_height = float(payload.get("piece_height", DEFAULT_PIECE_HEIGHT))
    crop_mode = str(args.crop_mode or payload.get("crop_mode", PROJECTED_CROP_MODE))
    ...
    mask_module = load_mask_module() if args.mask else None
    ...
    for record in eval_records:
        ...
        if mask_module is not None:
            mask_builder = mask_module.GeometricMaskBuilder(
        ...
        for square_index in range(64):
            square_row = square_index // 8
            square_col = square_index % 8
            crop = extract_study_piece_crop(
                image,
                record.corners,
                row=square_row,
                col=square_col,
            ...
            if mask_builder is not None:
                crop = mask_builder.apply_to_crop(crop, square_index=square_index)
            crops.append(preprocess_square_crop(crop, size=input_size, augment=False))
        batch = torch.stack(crops, dim=0).to(device)
        with torch.no_grad():
            type_logits, base_logits = model(batch)
        predicted_labels = tuple(
            int(value) for value in infer_square_labels(type_logits, base_logits).cpu().tolist()
        )
        predicted_pieces = labels_to_piece_tuples(predicted_labels)
    ...
    results_path.write_text(render_results_markdown(category_metrics, args.mask, crop_mode))
...
def load_mask_module():
    mask_path = _PROJECT_ROOT / "study" / "geometric-mask" / "mask.py"
    module_name = "study_geometric_mask"
    spec = importlib.util.spec_from_file_location(module_name, mask_path)
    if spec is None or spec.loader is None:
        raise ValueError(f"Failed to load geometric mask module from {mask_path}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate the base-head study on study/eval.")
    ...
    parser.add_argument("--crop-mode", choices=CROP_MODES, default=None)
    ...
    parser.add_argument("--results-path", type=Path, default=_THIS_DIR / "RESULTS.md")
    return parser
